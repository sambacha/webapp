...
N101 is assigned a non-built-in XSD data type consisting of the literal "DE_" and the data element number. 
The resulting DE_98 is used to define the available internal code values and their descriptions. 
...
In the example above, DE_98 is assigned a non-built-in XSD data type of ID.
...
The following example is for that segment. Not the entire segment is illustrated, just enough to point out how 
 a code list for the first data element is specified.
