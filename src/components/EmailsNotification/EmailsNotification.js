...
const EmailsNotification = () => {
  ...
  const emailArray = emails.filter(email => email.verified).map(email => email.email);

  return (
    <InlineNotification>
      Using GitHub verified email addresses <strong>{emailArray.join(`, `)}</strong>. If you'd like
      to use additional email addresses with Carbon badges, please{` `}
      <a
